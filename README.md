...
- A **private aggregate device** clocked by the real output device carries the tap; an input IOProc drains it into a lock-free ring, and an output IOProc on the real device applies the DSP and plays it. Both callbacks run on the same clock, so the ring only absorbs their phase offset. A lightweight **watchdog** rebuilds the graph if the tap ever falls silent mid-playback (and, if the music-only tap can't deliver on a given machine, quietly reverts to the system-wide one).
